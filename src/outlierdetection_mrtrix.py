...
parser.add_argument("--bgrad", required=True, help="Path to mrtrix gradients file")
...
def gmm_weighting(fdmri, fspred, mask, bgrad, outpath, filename_gmm):
...
# for mrtrix grad table 8/6/2026
grad_data = np.loadtxt(fbgrad)
bvecs = grad_data[:, :3]
bvals = grad_data[:, 3]
gtab = gradient_table(bvals=bvals, bvecs=bvecs)


print("bgrad.shape: ", gtab)
...
if args.fsliceweights_gmmodel is not None and fspred is not None:
    ...
    gmm_weighting(fdmri=fdmrigmm, fspred=fspredgmm, mask=maskgmm, bgrad=fbgrad, outpath=outpath, filename_gmm=args.fsliceweights_gmmodel)
else:
