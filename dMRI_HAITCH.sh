    ...
    if [[ -f "${QUAL_CONTROL_DIR}/${SUBJECTID}_SegmStep4_outliers_volume_index.txt" && $NOOVER = 1 ]] ; then echo "Step $STEPX output found, skipping." 
        ...
        if [[ $SPLIT_INTO_ODDEVEN == "YES" ]]; then
            ...
            for ((VIDX=0; VIDX<${NVOLUMES}; VIDX++)); do
                ...
                mrconvert -coord 3 $VIDX "${OUTPATHSUB}/working_odd.mif"   "${SEGMENTATION_DIR}/working_odd_TE${TE}_v${VNUM}.nii.gz" -quiet -force
                mrconvert -coord 3 $VIDX "${OUTPATHSUB}/working_even.mif" "${SEGMENTATION_DIR}/working_even_TE${TE}_v${VNUM}.nii.gz" -quiet -force
            done
        fi
...
if [[ ${FEDI_DMRI_PIPELINE_STEPS["STEP6_SLICECORRECTDISTORTION"]} == "TODO" ]] && [[ $DWIMODALITY == "dwiME" ]] && [[ $NUMBER_ECHOTIME -gt 1 ]] && [[ ! -e ${LOCK6} ]] ; then
    ...
    ##########################################################################################################################################################
        ...
        if [[ $DISTORTIONCORRECTION_WAY == "CLASSIC" ]]; then
            ...
            if [[ -f ${DISTORTIONCO_DIR}/TOPUP/dwidc_TOPUP_CLASSIC_TE1_$DIRSORDER.nii.gz && -f ${DISTORTIONCO_DIR}/TOPUP/dwidc_TOPUP_CLASSIC_TE1_$DIRSORDER.nii.gz && $NOOVER = 1 ]] ; then
                echo "CLASSIC distortion correction output found, skipping"
            else
            ...
            fi # end of check-for-output if statement

        elif [[ $DISTORTIONCORRECTION_WAY == "VOLUME" ]]; then

            if [[ -f ${DISTORTIONCO_DIR}/TOPUP/dwidc.nii.gz && $NOOVER = 1 ]] ; then
                echo "VOLUME TOPUP output found, skipping."
            else
            ...
            fi # end of check-for-output

        elif [[ $DISTORTIONCORRECTION_WAY == "VOLUMETOPUPONLY" ]]; then
            if [[ -f ${DISTORTIONCO_DIR}/TOPUP/dwidcTE1_${DISTORTIONCORRECTION_WAY}_${DIRSORDER}.nii.gz && -f ${DISTORTIONCO_DIR}/TOPUP/dwidcTE2_${DISTORTIONCORRECTION_WAY}_${DIRSORDER}.nii.gz && $NOOVER = 1 ]] ; then
                echo "VOLUME TOPUP ONLY output found, skipping."
            else
            ...
            if [[ -f "${DISTORTIONCO_TMP}/dwidcTOPUPs_perSlice.nii.gz" && $NOOVER = 1 ]] ; then
                echo "SLICE TOPUP output found, skipping."
            else
            ...
            if [[ -f "${DISTORTIONCO_DIR}/TOPUP/dwidc_TE1_SliceDupTopup_${DIRSORDER}.nii.gz" && -f "${DISTORTIONCO_DIR}/TOPUP/dwidc_TE1_SliceDupTopup_${DIRSORDER}.nii.gz" && $NOOVER = 1 ]] ; then
                echo "SLICE TOPUP ONLY output found, skipping."
            else
    ...
    if [[ -f "${MOTIONCORREC_DIR}/spred_final.nii.gz" && $NOOVER = 1 ]] ; then echo "Step $STEPX output found, skipping." 
        ...
        for ((ITER=0; ITER<$EPOCHS; ITER++)); do
            ...
            if [[ $SHARDCON = 1 ]] ; then
                ...
                singularity exec docker://arfentul/shard-recon:latest /bin/bash -c " python ${SRC}/outlierdetection_mrtrix.py --dmri  "$WORKING_DMRI" --dmrigmm  "$WORKING_DMRI_GMM" \
                    --bgrad "${GRAD4CLS}" --bval "$BVALSTE" --bvec "$BVECSTE" --outpath "${SLICEWEIGHTS_DIR}" \
                    --fsliceweights_mzscore  "fsliceweights_mzscore_${ITER}.txt" --fsliceweights_angle_neighbors "fsliceweights_angle_neighbors_${ITER}.txt" \
                    --fsliceweights_corre_neighbors "fsliceweights_corre_neighbors_${ITER}.txt" --fsliceweights_gmmodel "fsliceweights_gmmodel_${ITER}.txt" \
                    --fvoxelweights_shorebased "fvoxelweights_shore_${ITER}.nii.gz" --spred "${MOTIONCORREC_DIR}/spred${ITERM}.nii.gz" --spredgmm "${SPRED_GMM}" \
                    --mask "$WORKING_DMRIMASK" --maskgmm "$WORKING_DMRIMASK_GMM" "

            else echo "(original) script outlier detection"
